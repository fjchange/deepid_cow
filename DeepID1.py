...
testX1, testX2,testY,validX,validY,  trainX, trainY = load_data.load_data()
...
if __name__ == '__main__':
    def get_batch(data_x, data_y, start):
        end = (start + 1024) % data_y.shape[0]
        if start < end:
            return data_x[start:end], data_y[start:end], end
        return np.row_stack([data_x[start:], data_x[:end]]), np.row_stack([data_y[start:], data_y[:end]]).reshape((-1,class_num)), end


    data_x = np.array(trainX)
    data_y=np.eye(class_num,dtype=np.float32)[trainY]
    #data_y = (np.arange(class_num) == trainY[:, None]).astype(np.float32)
    #validY = (np.arange(class_num) == validY[:, None]).astype(np.float32)
    validX=np.array(validX)
    val_y=np.eye(class_num,dtype=np.float32)[validY]
    validY=val_y.reshape((-1,class_num))
    ...
    for i in range(50001):
        ...
        batch_y=batch_y.reshape((-1,class_num))
        ...
        if i % 100 == 0:
            ...
            print("--------------->  training at step",i, "accuracy is ",accu)
        if i % 5000 == 0 and i != 0:
